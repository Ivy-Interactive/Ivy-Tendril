...
public sealed class GeminiFailureAnalyzer : IFailureAnalyzer
{
    public FailureAnalysis Analyze(FailureContext context)
    {
        if (context.TimedOut)
        {
            return new FailureAnalysis
            {
                ...
                Reason = context.IdleTimeout
                    ? "Gemini went idle beyond the configured threshold"
                    : "Gemini exceeded the total timeout",
                IsRetryable = true,
        ...
        if (ContainsAny(stderr, "not logged in", "oauth", "unauthorized", "401", "403", "sign in"))
        {
            return new FailureAnalysis
            {
                ...
                ContextLines = context.StderrLines,
                IsRetryable = false,
                Suggestion = "Run 'gemini auth' to re-authenticate",
            };
        }

        if (ContainsAny(stderr, "model not found", "invalid model", "not available"))
        {
            return new FailureAnalysis
            {
                Kind = FailureKind.InvalidModel,
                Reason = "The requested model is not available",
                ContextLines = context.StderrLines,
                IsRetryable = false,
