...
- Read `plan.yaml` via `tendril plan get <plan-id>` and the latest revision from `Revisions/` (highest numbered .md file) together in a single batched read operation to reduce spin-up overhead
