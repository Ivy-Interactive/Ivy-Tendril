...
- **Batch sequential reads.** Batch sequential file, plan, or memory reads into a single read operation (e.g. passing multiple file paths in one tool call or reading multiple memory files with `tendril promptware read-memory <name> file1.md file2.md`) to minimize agent spin-up overhead and latency.
