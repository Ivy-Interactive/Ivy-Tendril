...
public class PlanAnnotationServiceTests : IDisposable
{
    private readonly string _testDir;
    private readonly PlanAnnotationService _service;

    public PlanAnnotationServiceTests()
    {
        _testDir = Path.Combine(Path.GetTempPath(), "tendril_draft_annotations_test_" + Guid.NewGuid().ToString("N"));
        Directory.CreateDirectory(_testDir);
        _service = new PlanAnnotationService(NullLogger<PlanAnnotationService>.Instance);
    }
