...
public class DraftDiffCommentServiceTests : IDisposable
{
    private readonly string _testDir;
    private readonly DraftDiffCommentService _service;

    public DraftDiffCommentServiceTests()
    {
        _testDir = Path.Combine(Path.GetTempPath(), "tendril_draft_diff_test_" + Guid.NewGuid().ToString("N"));
        Directory.CreateDirectory(_testDir);
        _service = new DraftDiffCommentService(NullLogger<DraftDiffCommentService>.Instance);
    }
