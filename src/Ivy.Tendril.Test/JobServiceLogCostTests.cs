...
public class JobServiceLogCostTests : IDisposable
{
    ...
    {
        JobService.LogCostToCsv(_tempDir.Path, "ExecutePlan", 150000, 0.4500m, "claude-opus-5", "agent", "claude");
        ...
        Assert.Equal("ExecutePlan,150000,0.4500,claude-opus-5,agent,claude", lines[1]);
    }
    ...
    {
        JobService.LogCostToCsv(_tempDir.Path, "ExecutePlan", 150000, 0.4500m, "claude-opus-5", "agent", "claude");
        JobService.LogCostToCsv(_tempDir.Path, "CreatePlan", 25000, 0.0750m, "claude-sonnet-5", "computed", "claude");

        var lines = ReadCsv();
        Assert.Equal(3, lines.Length);
        Assert.Equal("Promptware,Tokens,Cost,Model,CostSource,Agent", lines[0]);
        Assert.Equal("ExecutePlan,150000,0.4500,claude-opus-5,agent,claude", lines[1]);
        Assert.Equal("CreatePlan,25000,0.0750,claude-sonnet-5,computed,claude", lines[2]);
    }
    ...
    public void LogCostToCsv_ExistingThreeColumnFile_KeepsItsHeader()
    {
        ...
        JobService.LogCostToCsv(_tempDir.Path, "CreatePr", 99999, 1.2346m, "claude-opus-5", "agent", "claude");
        ...
        Assert.Equal("CreatePr,99999,1.2346,claude-opus-5,agent,claude", lines[2]);
    }
}
