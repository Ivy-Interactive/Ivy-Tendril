    ...
    string nextButtonText = "Next") : ViewBase
{
    public override object Build()
    {
        ...
        return Layout.Vertical().Gap(4).Margin(0, 0, 0, 20)
               ...
               | Text.Muted("Review and edit the verifications and review actions generated for your project.")
               | (Layout.Vertical().Gap(2)
                  | Text.Bold("Verifications")
                  | Text.Muted("Steps that run after each plan execution to validate changes.").Small()
               ...
               | new Separator()
               | (Layout.Vertical().Gap(2)
                  | Text.Bold("Review Actions")
                  | Text.Muted("Commands that run during plan review (e.g., tests, linting).").Small()
