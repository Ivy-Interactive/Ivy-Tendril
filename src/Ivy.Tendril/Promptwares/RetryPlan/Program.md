...
Re-execute an approved plan in isolated git worktrees, incorporating reviewer feedback from a prior execution.
...
- **ChangeRequest** — Reviewer feedback describing what needs to change in this re-execution. Address this feedback as your primary objective.
...
**Note:** This is a re-execution. The plan has already been executed once, but the reviewer was not satisfied with the result and has provided feedback in the `ChangeRequest` header. A previous worktree branch may exist — handle this gracefully (delete old worktree first, or create with a new branch suffix). Check for existing artifacts and verification reports from prior runs.

## Change Request Priority

This is a re-execution triggered by reviewer feedback. The `ChangeRequest` header contains specific changes the reviewer wants. Your primary objective is to address this feedback.
...
When implementing, focus on what the reviewer asked to change. You may need to modify the previous execution's approach, fix issues, add missing functionality, or take a different direction entirely based on the feedback.
...
**Note:** The JobService also performs this check before launching RetryPlan, but this step acts as a safety net in case the dependency state changed between job launch and execution.
...
1. **Check each repo is not itself a worktree** — If `<repo-path>/.git` is a file containing `gitdir:`, the repo is a worktree. Fail with error:
   > ERROR: Repository at <repo-path> is itself a worktree. RetryPlan cannot create worktrees inside worktrees. Update project configuration to use the main repo path.
...
if [ -f .git ] && grep -q "gitdir:" .git; then
    ...
    echo "RetryPlan cannot create worktrees inside worktrees."
...
5. **Apply sync strategy** — REQUIRED after worktree creation and `.git` file verification:
   ...
   **Error handling:**
   - If the command fails (non-zero exit code), the entire RetryPlan run should fail
...
Work exclusively in the worktree directories. Follow the plan's latest revision, **prioritizing the ChangeRequest feedback**:

1. **ChangeRequest** — Address the reviewer's feedback first. This is why this re-execution was triggered.
2. **Problem** — Understand what needs to be done (from the plan revision)
3. **Solution** — Execute the implementation steps in the worktree, incorporating the ChangeRequest modifications
4. **Tests** — Write and run all tests specified in the plan
...
Worktrees are **not** cleaned up by RetryPlan. They remain on disk so that CreatePr can push branches and create PRs directly from the worktree.
