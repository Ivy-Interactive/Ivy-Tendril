...
public static class FirmwareCompiler
{
    ...
    public static string Compile(FirmwareContext context)
    {
        ...
        // C# Fallback logic
        {
            ...
            var memoryDir = Path.Combine(context.ProgramFolder, "Memory");
            if (!Directory.Exists(memoryDir))
            {
                memoryDir = PromptwareHelper.ResolveMemoryDirectory(promptwareName, tendrilHome);
            }
            var memoryListing = ListMemoryFiles(memoryDir, "(no memory yet)");
